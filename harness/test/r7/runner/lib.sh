...
# Generic R7 Docker mechanics. Case semantics belong in testdata/r7/cases.

set -euo pipefail

R7_RUNNER_DIR=$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd -P)
R7_HARNESS_ROOT=$(cd "$R7_RUNNER_DIR/../../.." && pwd -P)
R7_REPOSITORY_ROOT=$(cd "$R7_HARNESS_ROOT/.." && pwd -P)
...
r7_build_image() {
  docker build --quiet -f "$R7_HARNESS_ROOT/test/r7/docker/Dockerfile" \
    -t "$R7_IMAGE" "$R7_HARNESS_ROOT" >/dev/null
  R7_IMAGE_ID=$(docker image inspect --format '{{.Id}}' "$R7_IMAGE")
  R7_BINARY_DIGESTS=$(docker run --rm --entrypoint sha256sum "$R7_IMAGE" \
    /usr/local/bin/mnemon-harness /usr/local/bin/mnemond)
...
r7_begin_case() {
  ...
  while IFS= read -r node; do
    ...
    test "$(docker exec "$container" sha256sum /usr/local/bin/mnemon-harness \
      /usr/local/bin/mnemond)" = "$R7_BINARY_DIGESTS" || \
      r7_fail "node $node does not run the candidate binaries"
  done <<<"$R7_NODES"
  ...
  while IFS= read -r origin; do
    while IFS= read -r remote; do
      ...
      docker exec -i -w /workspace "$(r7_container "$origin")" \
        mnemon-harness peer enroll --alias "$remote" --project-root /workspace \
...
r7_read_artifact() {
  ...
  r7_exec "$node" mnemon-harness artifact read "$handle"
}
