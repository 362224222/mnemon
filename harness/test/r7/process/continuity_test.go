...
func TestHandlingSurvivesProcessAndDaemonBoundaries(t *testing.T) {
	...
	intent := fmt.Sprintf(
		...
		kind, payload)
	receipt := runTerminal(t, fixture.harness, fixture.workspace, intent, "agent", "submit", "--json")
	...
	replayed := runTerminal(t, fixture.harness, fixture.workspace, "", "agent", "current", "--json")
...
func newProcessFixture(t *testing.T) *processFixture {
	...
	fixture := &processFixture{
		harness:   filepath.Join(binDirectory, "mnemon-harness"),
		daemon:    filepath.Join(binDirectory, "mnemond"),
		workspace: shortWorkspace(t),
	}
	buildBinary(t, root, "./cmd/mnemon-harness", fixture.harness)
	buildBinary(t, root, "./cmd/mnemond", fixture.daemon)
	...
	fixture.active = startDaemon(t, fixture.daemon, fixture.state)
...
func parseView(t *testing.T, raw string) viewProjection {
	...
	if view.Schema != "mnemon.agent.view" || view.Version != 4 || view.View == "" {
...
func moduleRoot(t *testing.T) string {
	...
	if _, err := os.Stat(filepath.Join(root, "go.mod")); err != nil {
		t.Fatalf("resolved Harness module root %q is invalid: %v", root, err)
	}
	return root
}
