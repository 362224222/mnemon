...
func factEvidenceInput(fact factRecord) Fact {
	return Fact{Kind: fact.Kind, Causes: slices.Clone(fact.Causes), References: References{
		...
		Selection: fact.Refs.Selection,
	}, Fields: FactFields{
		Action: fact.Facts.Action, Alpha: fact.Facts.Alpha,
		...
		HookCue: fact.Facts.HookCue, InvalidVotes: fact.Facts.InvalidVotes,
		InvalidCount: fact.Facts.InvalidCount, MarginAfter: fact.Facts.MarginAfter,
		MarginBefore: fact.Facts.MarginBefore, NoVote: fact.Facts.NoVote,
		NoVotes: fact.Facts.NoVotes, OccupiedClaims: fact.Facts.OccupiedClaims,
		...
		Phase: fact.Facts.Phase, PreferenceAfter: fact.Facts.PreferenceAfter,
		PreferenceBefore: fact.Facts.PreferenceBefore, Recolored: fact.Facts.Recolored,
		Replayed: fact.Facts.Replayed, ReplyRequired: fact.Facts.ReplyRequired,
		Result: fact.Facts.Result, RelatedProjected: fact.Facts.RelatedProjected,
		RelatedTotal: fact.Facts.RelatedTotal, Role: fact.Facts.Role,
		Round: fact.Facts.Round, SampleSize: fact.Facts.SampleSize,
