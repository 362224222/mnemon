...
func allowedHarnessImport(source, target string) bool {
	...
	allowed := map[string]map[string]bool{
		...
		"node":     {"event": true, "store": true, "agent": true, "artifact": true, "peer": true},
...
func knownHarnessPackage(name string) bool {
	switch name {
	case "cmd", "assets", "agent", "artifact", "event", "integration", "localapi", "model", "node", "peer", "store", "teamwork", "testkit":
		return true
	default:
		return false
	}
