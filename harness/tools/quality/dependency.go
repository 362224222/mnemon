...
func allowedHarnessImport(source, target string) bool {
	if target == "model" && knownHarnessPackage(source) &&
		source != "agency" && source != "selector" {
		return true
	}
	allowed := map[string]map[string]bool{
		"cmd":      {"integration": true, "localapi": true, "node": true},
		"node":     {"event": true, "store": true, "agent": true, "artifact": true, "peer": true, "agency": true, "selector": true},
...
func knownHarnessPackage(name string) bool {
	switch name {
	case "cmd", "assets", "agency", "agent", "artifact", "event", "integration", "localapi", "model", "node", "peer", "selector", "store", "teamwork", "testkit":
		return true
	default:
		return false
	}
