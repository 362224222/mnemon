...
func TestConfiguredSnapshotIncludesConfiguredRoles(t *testing.T) {
	...
	for _, name := range []string{"-watch", "-drive", "-display"} {
...
func TestConfiguredSnapshotKeepsMnemonhubAsExchangeWatcher(t *testing.T) {
	...
	if _, ok := snapshot.Workers["mnemonhub-display"]; ok {
...
func TestRoleDetailsDescribeDaemonBoundaries(t *testing.T) {
	...
	for _, want := range []ConfiguredRoleDetail{
		...
		{WorkerName: "multica-display", Kind: WorkerSurface, Label: "surface", Value: productconfig.ConnectionMultica, Boundary: "display-surface"},
	} {
...
func TestRoleSummaryReflectsConfiguredDaemonRoles(t *testing.T) {
	...
	got := RoleSummary(cfg)
	if got.InteractionWatchers != 2 || got.DriveSources != 1 || got.DisplaySurfaces != 1 {
