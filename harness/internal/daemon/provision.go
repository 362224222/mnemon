...
func ensureProvisionDirectories(root string) (string, error) {
	...
	harnessDirectory := filepath.Join(mnemonDirectory, "harness")
	if err := ensureOwnedDirectory(harnessDirectory, true); err != nil {
		return "", err
	}
	stateDirectory := filepath.Join(harnessDirectory, "node")
