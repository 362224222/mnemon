...
func InspectAuthority(ctx context.Context, workspace string) (_ AuthoritySnapshot, err error) {
	return InspectAuthorityWithCredentials(ctx, workspace, nil)
}

func InspectAuthorityWithCredentials(ctx context.Context, workspace string,
	credentials ProfileCredentialAuthority,
) (_ AuthoritySnapshot, err error) {
	if ctx == nil {
		return AuthoritySnapshot{}, fmt.Errorf("%w: context is unavailable", ErrAuthorityInspection)
	}
	if contextErr := ctx.Err(); contextErr != nil {
		return AuthoritySnapshot{}, fmt.Errorf("%w: %w", ErrAuthorityInspection, contextErr)
	}
	...
	if openErr != nil {
		return AuthoritySnapshot{}, fmt.Errorf("%w: %w", ErrAuthorityInspection, openErr)
	}
	defer func() {
...
func authoritySnapshot(authority store.LocalAuthority) AuthoritySnapshot {
	return AuthoritySnapshot{Host: authority.Profile.Host(), Runtime: authority.Profile.Runtime(),
