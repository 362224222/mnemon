...
func TestControllerAdmissionServiceWrapsEveryManagedRoute(t *testing.T) {
	...
	if _, apiErr := service.HookCheck(ctx, metadata, HookCheckRequest{}); apiErr != nil {
		t.Fatal(apiErr)
	}
	if _, apiErr := service.AgentCurrent(ctx, metadata, AgentCurrentRequest{}); apiErr != nil {
		t.Fatal(apiErr)
	}
	...
	checks := []func() *APIError{
		func() *APIError {
			_, apiErr := service.HookCheck(ctx, metadata, HookCheckRequest{})
			return apiErr
		},
		func() *APIError {
			_, apiErr := service.AgentCurrent(ctx, metadata, AgentCurrentRequest{})
			return apiErr
		},
		func() *APIError {
	...
	for index, check := range checks {
		if apiErr := check(); apiErr == nil || apiErr.Code != CodeMnemondUnavailable {
...
func TestControllerAdmissionServiceKeepsHandlerEnteredUntilStoreWorkReturns(t *testing.T) {
	...
	service := controllerAdmissionService{gate: gate, next: next}
	handlerDone := make(chan *APIError, 1)
	go func() {
		_, apiErr := service.HookCheck(context.Background(), RequestMetadata{},
			HookCheckRequest{})
		handlerDone <- apiErr
	}()
	select {
...
func (service *blockingAdmissionService) HookCheck(context.Context, RequestMetadata,
	HookCheckRequest,
...
func (*blockingAdmissionService) AgentCurrent(context.Context, RequestMetadata,
	AgentCurrentRequest,
) (AgentCurrentResponse, *APIError) {
	return AgentCurrentResponse{}, NewAPIError(CodeInternal,
		"unexpected AgentCurrent")
}

func (*blockingAdmissionService) TeamworkAction(context.Context, RequestMetadata,
	TeamworkActionRequest,
) (OperationResponse, *APIError) {
	return OperationResponse{}, NewAPIError(CodeInternal,
		"unexpected TeamworkAction")
}

func (*blockingAdmissionService) AgentResolve(context.Context, RequestMetadata,
	AgentResolveRequest,
) (OperationResponse, *APIError) {
	return OperationResponse{}, NewAPIError(CodeInternal,
...
func (service *recordingAdmissionService) HookCheck(context.Context, RequestMetadata,
	HookCheckRequest,
) (HookCheckResponse, *APIError) {
	service.calls.Add(1)
	return HookCheckResponse{}, nil
...
func (service *recordingAdmissionService) AgentCurrent(context.Context, RequestMetadata,
	AgentCurrentRequest,
) (AgentCurrentResponse, *APIError) {
	service.calls.Add(1)
...
func (service *recordingAdmissionService) TeamworkAction(context.Context, RequestMetadata,
	TeamworkActionRequest,
) (OperationResponse, *APIError) {
	service.calls.Add(1)
...
func (service *recordingAdmissionService) AgentResolve(context.Context, RequestMetadata,
	AgentResolveRequest,
) (OperationResponse, *APIError) {
	service.calls.Add(1)
	return OperationResponse{}, nil
