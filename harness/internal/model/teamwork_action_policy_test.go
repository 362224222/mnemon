...
func TestTeamworkActionPolicyRejectsIncompleteOrUnsafeProjection(t *testing.T) {
	t.Parallel()
	...
	assertInvalidTeamworkActionPolicy(t, "multi-result non-offer",
		func(spec *TeamworkActionPolicySpec) { spec.Entries[1].MaxResults = 2 })
...
func TestTeamworkActionPolicyDerivesEventAndPreservesAssetPolicy(t *testing.T) {
	t.Parallel()
	...
	offer, offerOK := policy.Operation(OperationTeamworkOffer)
	accept, acceptOK := policy.Operation(OperationTeamworkAccept)
	decline, declineOK := policy.Operation(OperationTeamworkDecline)
	if !offerOK || !acceptOK || !declineOK || offer.MaxResults() != 2 ||
		accept.EventType() != EventReviewAcceptRequested || decline.EventType() != EventReviewDeclineRequested {
		t.Fatalf("derived policy entries = (%#v, %#v, %#v)", offer, accept, decline)
	}
	...
	if err != nil || !reflect.DeepEqual(actions,
		...
		t.Fatalf("asset-owned ActionsForContexts() = (%v, %v)", actions, err)
	}
}
