...
// AuthorityProvider re-reads the singleton Node and Teamwork Profile behind
// the authenticated local controller. Implementations must not cache this
// snapshot or derive it from request content.
type AuthorityProvider interface {
	Authority(context.Context, RequestMetadata) (AuthoritySnapshot, *APIError)
}

// AuthorityProviderFunc adapts one controller-owned authority read.
...
func NewAuthorityResponse(snapshot AuthoritySnapshot) (AuthorityResponse, error) {
	...
	updatedAt, err := canonicalAuthorityTime(snapshot.UpdatedAt)
	if err != nil {
