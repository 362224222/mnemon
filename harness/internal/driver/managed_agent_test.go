...
type managedTurnClientFunc func(context.Context, string) (ManagedTurnResult, error)

func (f managedTurnClientFunc) StartTurn(ctx context.Context, query string) (ManagedTurnResult, error) {
	return f(ctx, query)
}

func TestManagedAgentDriverUsesSentinelOnly(t *testing.T) {
	var gotQuery string
...
func TestManagedAgentDriverRejectsRemotePrincipal(t *testing.T) {
	driver := &ManagedAgentDriver{
		Principal: "codex-a@project",
		Client:    managedTurnClientFunc(func(context.Context, string) (ManagedTurnResult, error) { return ManagedTurnResult{}, nil }),
		Ledger:    NewMemoryManagedWakeLedger(),
	}
...
func TestManagedWakeCandidatesFromDerivedEvents(t *testing.T) {
