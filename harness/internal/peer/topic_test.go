...
func TestGossipSessionCloseSerializesValidatorRecreation(t *testing.T) {
	fixture := newTopicLifecycleFixture(t, "gossip-lifecycle")
	local, remote, channel := fixture.local, fixture.remote, fixture.channel
	ctx, gossip, session := fixture.ctx, fixture.gossip, fixture.session
	var err error
	...
	writerPending := waitTopicGateWriter(session.gate, 2*time.Second)
	...
	reactivationWriterPending := waitTopicGateWriter(stableGate, 2*time.Second)
	...
	secondWriterPending := waitTopicGateWriter(stableGate, 2*time.Second)
...
func waitTopicGateWriter(gate *channelGate, timeout time.Duration) bool {
...
func newTopicLifecycleFixture(t *testing.T, name string) topicLifecycleFixture {
	t.Helper()
	local := testAuthorityPeer(t, name+"-local")
	remote := testAuthorityPeer(t, name+"-remote")
	channel := testAuthorityChannel(t, name+"-channel", model.BindingActive, local, remote)
	authority, _ := NewAuthority(local.modelID)
	if err := authority.Replace(NetworkAuthoritySnapshot{LocalPeerID: local.modelID,
		Channels: []ChannelAuthoritySnapshot{channel}}); err != nil {
		t.Fatal(err)
	...
	return topicLifecycleFixture{local: local, remote: remote, channel: channel,
		ctx: ctx, gossip: gossip, session: session}
}
