...
func TestArtifactReceiverRealStoreCASClientLibp2pHappyAndRestart(t *testing.T) {
	...
	server, err := NewArtifactServer(context.Background(), ArtifactServerOptions{
		Host: originHost, Source: source, CAS: serverContent.cas(),
	})
	if err != nil {
		t.Fatal(err)
	}
	defer server.Close()
	client, err := NewArtifactClient(ArtifactClientOptions{Host: requesterHost})
	if err != nil {
		t.Fatal(err)
	}
	cas := newArtifactReceiverTestCAS(t)
...
func TestArtifactReceiverCASUseLeaseCoversReadyFailureAndCancellation(t *testing.T) {
	t.Run("nil use lease fails closed", func(t *testing.T) {
		at := artifactReceiverTestTime()
		origin := testkit.NewIdentity(t, "artifact-receiver-lifecycle-nil-origin")
		channelID := artifactReceiverChannelID(t, "lifecycle-nil")
		content := artifactReceiverContentForTest(t, "lifecycle-nil", []byte("nil lifecycle"))
		backend := &artifactReceiverTestBackend{claims: []artifactReceiverClaim{
			artifactReceiverClaimForTest(t, "lifecycle-nil", origin, channelID,
				[]model.Digest{content.manifest.RootDigest()}),
		}}
		...
		receiver := newArtifactReceiverForTest(t, backend, client, cas, 10*time.Second, 1,
			&artifactReceiverTestClock{at: at})
		err := <-runArtifactReceiverForTest(receiver, context.Background())
		if !errors.Is(err, ErrArtifactReceiverInvariant) ||
			receiver.Snapshot().FatalCode != ArtifactReceiverFatalCASInvariant {
	...
	t.Run("ready settlement", func(t *testing.T) {
		at := artifactReceiverTestTime()
		origin := testkit.NewIdentity(t, "artifact-receiver-lifecycle-ready-origin")
		channelID := artifactReceiverChannelID(t, "lifecycle-ready")
		content := artifactReceiverContentForTest(t, "lifecycle-ready", []byte("ready lifecycle"))
		inner := &artifactReceiverTestBackend{claims: []artifactReceiverClaim{
			artifactReceiverClaimForTest(t, "lifecycle-ready", origin, channelID,
				[]model.Digest{content.manifest.RootDigest()}),
		}}
		releaseReady := make(chan struct{})
		backend := &artifactReceiverReadyGateBackend{artifactReceiverTestBackend: inner,
			entered: make(chan struct{}), release: releaseReady}
		cas := newArtifactReceiverTestCAS(t)
		receiver := newArtifactReceiverForTest(t, backend,
			artifactReceiverContentClient(t, content), cas, 10*time.Second, 1,
			&artifactReceiverTestClock{at: at})
		done := make(chan error, 1)
		go func() { done <- receiver.runCycle(context.Background()) }()
	...
	t.Run("fatal receive failure", func(t *testing.T) {
		at := artifactReceiverTestTime()
		origin := testkit.NewIdentity(t, "artifact-receiver-lifecycle-fatal-origin")
		channelID := artifactReceiverChannelID(t, "lifecycle-fatal")
		content := artifactReceiverContentForTest(t, "lifecycle-fatal", []byte("fatal lifecycle"))
		backend := &artifactReceiverTestBackend{claims: []artifactReceiverClaim{
			artifactReceiverClaimForTest(t, "lifecycle-fatal", origin, channelID,
				[]model.Digest{content.manifest.RootDigest()}),
		}}
	...
	t.Run("cancelled ready settlement", func(t *testing.T) {
		at := artifactReceiverTestTime()
		origin := testkit.NewIdentity(t, "artifact-receiver-lifecycle-cancel-origin")
		channelID := artifactReceiverChannelID(t, "lifecycle-cancel")
		content := artifactReceiverContentForTest(t, "lifecycle-cancel", []byte("cancel lifecycle"))
		inner := &artifactReceiverTestBackend{claims: []artifactReceiverClaim{
			artifactReceiverClaimForTest(t, "lifecycle-cancel", origin, channelID,
				[]model.Digest{content.manifest.RootDigest()}),
		}}
		backend := &artifactReceiverReadyGateBackend{artifactReceiverTestBackend: inner,
