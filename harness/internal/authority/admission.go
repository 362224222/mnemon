...
func (s *Store) commitAdmission(ctx context.Context, proof AttachmentProof,
	...
	handlingIDs, err := newHandlingIDs(len(request.Targets()))
	if err != nil {
		return AdmissionResult{}, err
	}
...
func commitAcceptedAdmissionTx(ctx context.Context, tx *sql.Tx,
	...
	event, err := agency.NewEvent(request, agency.EventStamp{ID: eventID,
		AcceptedAt: now, OriginSequence: sequence})
	if err != nil {
		return AdmissionResult{}, fmt.Errorf("admit Intent: construct Event: %w", err)
	}
	if err := insertEventTx(ctx, tx, event); err != nil {
		return AdmissionResult{}, err
	}
	if err := applyLocalEffectTx(ctx, tx, request, event, handlingIDs); err != nil {
		return AdmissionResult{}, err
	}
...
var (
	...
	rejectionRemoteUnsupported   = mustRejectionCode("authority.remote_unsupported")
