...
func deriveFreshCurrentProjection(ctx context.Context, tx *sql.Tx, handling model.Handling,
	...
	projection, err := model.NewCurrentProjection(model.CurrentProjectionSpec{
		...
		AllowedActions: deriveCurrentActions(authority.role, authority.event, authority.work, exactUpdate),
	})
...
func readFreshCurrentAuthority(ctx context.Context, tx *sql.Tx, handling model.Handling,
	...
	if parentResume {
		return freshCurrentAuthority{}, fmt.Errorf("%w: parent-resume requires bounded child_results", ErrCurrentReadUnsupported)
	}
...
func validateStoredCurrentAuthority(ctx context.Context, tx *sql.Tx,
	...
	if err := validateStoredCurrentWork(ctx, tx, receipt, event); err != nil {
		return nil, err
	}
...
func validateStoredCurrentEvent(ctx context.Context, tx *sql.Tx, projection model.CurrentProjection,
	...
	if parentResume {
		return model.Event{}, fmt.Errorf("%w: parent-resume receipt used the base projection", ErrCurrentReadInvariant)
	}
