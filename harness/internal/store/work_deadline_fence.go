...
func readWorkExpiryDeliveryFence(ctx context.Context, tx *sql.Tx, scope LocalAdmissionScope,
	...
	binding, confirmed, err := readWorkDeadlineAudienceBinding(ctx, tx, scope, target)
	if err != nil {
...
func validWorkExpiryCurrent(run model.AgentRun, current model.CurrentReadReceipt,
	...
	projected := current.Projection().ActionWork()
	return hasHandling && current.RunID() == run.ID() && current.ProfileID() == run.ProfileID() &&
		...
		current.ActionWorkVersion() == work.Version() && projected.Version() == work.Version() &&
		projected.Iteration() == work.Iteration() && projected.DeadlineUnixNano() == work.DeadlineUnixNano() &&
		projected.State() == work.State() && projected.StateData().String() == work.StateData().String() &&
		!current.ReadAt().Before(work.UpdatedAt()) && !current.ReadAt().After(at) &&
