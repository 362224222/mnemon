...
func (s *Store) PrepareWorkExpiry(ctx context.Context, candidate WorkDeadlineCandidate,
	...
	defer tx.Rollback()
	scope, err := prepareLocalAdmissionScopeTx(ctx, tx, candidate.work.ChannelID(), 1, false)
	if err != nil {
...
func validateWorkExpiryPublicationTx(ctx context.Context, tx *sql.Tx,
	...
	publicKey, err := validateAdmissionAuthorityWithRequirements(ctx, tx, controller,
		model.Operation{}, preparation.acceptedAt, localAdmissionRequirements{})
	if err != nil {
...
func applyWorkExpiryEffects(ctx context.Context, tx *sql.Tx, scope LocalAdmissionScope,
	...
	if err := insertPublicationEvidenceWithDelivery(ctx, tx, event, status, reason); err != nil {
		return err
	}
