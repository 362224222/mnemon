...
// CheckpointVerifiedArtifactClosure atomically establishes immutable block
// rows, staged roots, exact root maps, and verified root promotion. This order
// is required by the schema seal: no map can be appended after verification.
func (s *Store) CheckpointVerifiedArtifactClosure(ctx context.Context,
	...
	if s == nil || s.db == nil || ctx == nil {
		return VerifiedArtifactClosureCheckpoint{}, errors.New("checkpoint Artifact closure: nil store or context")
	}
	...
	for _, root := range durableRoots {
		expected := rootBlocksForDigest(closure.RootBlocks, root.RootDigest)
		...
		if state.rootState == "staged" {
			...
			if err != nil || exactlyOne(result) != nil {
				return VerifiedArtifactClosureCheckpoint{}, fmt.Errorf("%w: promote root %s",
	...
	for index, root := range durableRoots {
		...
		if err != nil || !equalArtifactRootBlocks(storedMap,
			...
			return VerifiedArtifactClosureCheckpoint{}, fmt.Errorf("%w: verified root map differs",
				ErrArtifactConflict)
		}
	}
	if err := tx.Commit(); err != nil {
		return VerifiedArtifactClosureCheckpoint{}, fmt.Errorf("checkpoint Artifact closure: commit: %w", err)
	}
