...
func TestObserveAcceptanceRunReadsMnemondAndHubEvents(t *testing.T) {
	root := t.TempDir()
	...
	writeObserveTestMnemondDB(t, sharedDB, "codex-01@project", false)
	writeObserveTestMnemondDB(t, codexDB, "sync@local", true)
	writeObserveTestHubDB(t, hubDB)
	...
	report, err := observeAcceptanceRun(root, 5)
	if err != nil {
		t.Fatalf("observe acceptance run: %v", err)
	}
...
func TestAcceptanceObservationUsesSingleCommandSurface(t *testing.T) {
	...
	if !commands["observe"] {
		t.Fatalf("acceptance observe command is not registered")
	}
	for _, removed := range []string{"inspect", "watch"} {
		if commands[removed] {
			t.Fatalf("acceptance %s should be folded into acceptance observe", removed)
		}
	}
}

func writeObserveTestMnemondDB(t *testing.T, path, actor string, imported bool) {
	t.Helper()
	db := openObserveTestDB(t, path)
	defer db.Close()
	execObserveTestSQL(t, db, `CREATE TABLE events (ingest_seq INTEGER PRIMARY KEY AUTOINCREMENT, payload TEXT NOT NULL);`)
	execObserveTestSQL(t, db, `CREATE TABLE event_envelopes (seq INTEGER PRIMARY KEY AUTOINCREMENT, schema_version INTEGER NOT NULL, phase TEXT NOT NULL, event_id TEXT NOT NULL, event_type TEXT NOT NULL, subject TEXT NOT NULL, actor TEXT NOT NULL, audience TEXT NOT NULL DEFAULT '', correlation_id TEXT NOT NULL DEFAULT '', created_at TEXT NOT NULL DEFAULT '', decision_id TEXT NOT NULL DEFAULT '', envelope TEXT NOT NULL);`)
	execObserveTestSQL(t, db, `CREATE TABLE sync_events (origin_replica_id TEXT NOT NULL, local_decision_id TEXT NOT NULL, local_ingest_seq INTEGER NOT NULL, actor TEXT NOT NULL, correlation_id TEXT NOT NULL DEFAULT '', resource_kind TEXT NOT NULL, resource_id TEXT NOT NULL, resource_version INTEGER NOT NULL, fields_digest TEXT NOT NULL, fields TEXT NOT NULL, decided_at TEXT NOT NULL DEFAULT '', status TEXT NOT NULL DEFAULT 'pending', remote_peer_id TEXT NOT NULL DEFAULT '', acked_at TEXT NOT NULL DEFAULT '', diagnostic TEXT NOT NULL DEFAULT '', PRIMARY KEY(origin_replica_id, local_decision_id, resource_kind, resource_id));`)
	...
	if !imported {
		execObserveTestSQL(t, db, `INSERT INTO sync_events (origin_replica_id, local_decision_id, local_ingest_seq, actor, resource_kind, resource_id, resource_version, fields_digest, fields, status) VALUES ('local-a', 'dec-1', 1, ?, 'assignment', 'project', 1, 'sha256:test', '{}', 'synced')`, actor)
	}
}

func writeObserveTestHubDB(t *testing.T, path string) {
	t.Helper()
	db := openObserveTestDB(t, path)
	defer db.Close()
	execObserveTestSQL(t, db, `CREATE TABLE sync_remote_events (remote_seq INTEGER PRIMARY KEY AUTOINCREMENT, remote_peer_id TEXT NOT NULL, origin_replica_id TEXT NOT NULL, local_decision_id TEXT NOT NULL, local_ingest_seq INTEGER NOT NULL, actor TEXT NOT NULL, correlation_id TEXT NOT NULL DEFAULT '', resource_kind TEXT NOT NULL, resource_id TEXT NOT NULL, resource_version INTEGER NOT NULL, fields_digest TEXT NOT NULL, fields TEXT NOT NULL, decided_at TEXT NOT NULL DEFAULT '', received_at TEXT NOT NULL, status TEXT NOT NULL DEFAULT 'accepted', diagnostic TEXT NOT NULL DEFAULT '', UNIQUE(remote_peer_id, origin_replica_id, local_decision_id));`)
	execObserveTestSQL(t, db, `INSERT INTO sync_remote_events (remote_peer_id, origin_replica_id, local_decision_id, local_ingest_seq, actor, resource_kind, resource_id, resource_version, fields_digest, fields, decided_at, received_at, status) VALUES ('replica-01@hub', 'local-a', 'dec-1', 1, 'codex-01@project', 'assignment', 'project', 1, 'sha256:test', '{}', '2026-06-24T00:00:00Z', '2026-06-24T00:00:01Z', 'accepted')`)
}

func openObserveTestDB(t *testing.T, path string) *sql.DB {
	t.Helper()
...
func execObserveTestSQL(t *testing.T, db *sql.DB, query string, args ...any) {
	t.Helper()
