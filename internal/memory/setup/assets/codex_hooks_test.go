...
func isolatedBin(t *testing.T) string {
	...
	if runtime.GOOS == "windows" {
		// Windows cannot execute a symlink-only PATH: the shims carry no
		// .exe suffix, so CreateProcess resolves nothing and prime.sh
		// fails with "command not found". The isolated PATH is the whole
		// point of this test, so it cannot be faked on Windows; the
		// Codex prime hook is a POSIX shell asset. Skips here rather
		// than silently weakening the assertion.
		t.Skip("isolated symlink PATH is not executable on Windows; Codex prime.sh hooks are POSIX-only")
	}
	bin := t.TempDir()
