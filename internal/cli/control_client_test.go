...
func roundTripControlHandler(requests chan<- capturedControlRequest, credential []byte,
	...
	return http.HandlerFunc(func(writer http.ResponseWriter, request *http.Request) {
		...
		case routeCurrent:
			_, _ = io.WriteString(writer, `{"schema":"mnemon.agent.view","version":7,"view":"view:test","outstanding":{"open_total":0,"related_total":0,"related_projected":0,"truncated":false},"allowed_intents":[]}`+"\n")
		case routeSubmit:
...
func TestControlClientRejectsInvalidProjectionAndRemoteErrorEnvelope(t *testing.T) {
	...
	for name, response := range map[string]struct {
		...
		"duplicate projection": {Status: http.StatusOK,
			Body: `{"schema":"mnemon.agent.view","schema":"mnemon.agent.view","version":7}`},
